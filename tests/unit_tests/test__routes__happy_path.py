...
def test_delete_file(client: TestClient):
    ...
    # Delete file
    delete_response = client.delete(f"/v1/files/{TEST_FILE_PATH}")
    assert delete_response.status_code == 204

    # Verify deletion
    get_response = client.get(f"/v1/files/{TEST_FILE_PATH}")
    assert get_response.status_code == 404

    # Optionally, try to delete the file again to ensure it returns 404
    second_delete_response = client.delete(f"/v1/files/{TEST_FILE_PATH}")
